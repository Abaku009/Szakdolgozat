...
describe("Music recommendation integration test", () => {
    ...
    beforeEach(async () => {
        ...
        const lang = await pool.query(`
            ...
            VALUES ('English')
            RETURNING music_language_id
        `);
        language = lang.rows[0].music_language_id;
        ...
        const purchased = await pool.query(`
            ...
            RETURNING music_id
        `,[language,rockCategory,storage1]);

        purchasedMusicId = purchased.rows[0].music_id;

        await pool.query(`
            INSERT INTO music
            (title,price,format,performer,music_language_id,music_category_id,music_storage_id)
            VALUES ('Rock Song 2','10','digital','Band A',$1,$2,$3)
        `,[language,rockCategory,storage2]);

        await pool.query(`
            INSERT INTO music
            (title,price,format,performer,music_language_id,music_category_id,music_storage_id)
            VALUES ('Pop Song','10','digital','Artist B',$1,$2,$3)
        `,[language,popCategory,storage3]);
        ...
        await pool.query(`
            ...
            VALUES ($1,$2,1)
        `,[orderId,purchasedMusicId]);

    });

    test("should recommend music based on user order history", async () => {
